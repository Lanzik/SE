    ...
    public void handleEnterOrder(EnterOrderRq enterOrderRq) {
        try {
            ...
            if (!matchResult.getTrades().isEmpty()) {
                eventPublisher.publish(new OrderExecutedEvent(enterOrderRq.getRequestId(), enterOrderRq.getOrderId(), matchResult.getTrades().stream().map(TradeDTO::new).collect(Collectors.toList())));
