...
public class OrderBook {
    ...
    public void enqueueByQueue(Order order, List<Order> queue, Predicate<Order> condition) {
        ...
        ListIterator<Order> iterator = queue.listIterator();
        Order currentOrder;
        while (iterator.hasNext()) {
            currentOrder = iterator.next();
            if (condition.test(currentOrder)) {
                break;
            }
        }
    ...
    public void enqueueActiveStopLimitOrder(Order order) {
        List<Order> queue = getActiveStopLimitQueue(order.getSide());
        Predicate<Order> condition = order::inactiveOrderQueuesBefore;
        enqueueByQueue(order, queue, condition);
    }

    private List<Order> getActiveStopLimitQueue(Side side) {
        return getQueue(side);
    }
